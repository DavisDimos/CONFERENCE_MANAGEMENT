...
const PCmemberMiddleware = (req, res, next) => {
  try {
    ...
    if (decodedToken.roles && (decodedToken.roles.includes('PC_MEMBER') || decodedToken.roles.includes('PC_CHAIR'))||(decodedToken.roles.includes('AUTHOR'))) {
      ...
      console.log('Authorization failed. User does not have the "pc member" or "pc chair" or "author" role.');
      return res.status(401).json({ message: 'Authorization failed: Only PC member or PC chair or Author is allowed' });
    }
  } catch (err) {
...
module.exports = { PCmemberMiddleware };