...
const PCchairMiddleware = (req, res, next) => {
  try {
    ...
    if (decodedToken.roles && (decodedToken.roles.includes('PC_CHAIR') || decodedToken.roles.includes('PC_MEMBER'))|| (decodedToken.roles.includes('AUTHOR'))) {
      ...
      console.log('Authorization failed. User does not have the "pc chair" or "pc member" or "author" role.');
      return res.status(401).json({ message: 'Authorization failed: Only PC chair or PC member or Author is allowed' });
    }
  } catch (err) {
...
module.exports = { PCchairMiddleware };