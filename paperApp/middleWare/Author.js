...
// Verify the token using the JWT_KEY from the environment variables
    ...
    if (decodedToken.roles && (decodedToken.roles.includes('AUTHOR'))|| (decodedToken.roles.includes('PC_CHAIR'))||(decodedToken.roles.includes('PC_MEMBER'))) {
  ...
  // If the user does not have the "author" role, return an error response
      ...
      console.log('Authorization failed. User does not have the "author" or "pc chair" or "pc member"');
      return res.status(401).json({ message: 'Authorization failed: Only Author or PC chair or PC member is allowed' });
    }
  } catch (err) {
